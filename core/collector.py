...
class IntrinsicCollectStats(CollectStats):
    """A data structure for storing the statistics of the various rollouts.

    To Tianshou's original CollectStats we add tracking of the intrinsic rewards."""
...
class GoalCollector(Collector):
    ...
    def _collect(
        ...
        # kept for compatibility with Tianshou's existing code
        episode_returns: list[float] = []
        episode_lens: list[int] = []
        episode_start_indices: list[int] = []

        # to track the reward if we do not make enough steps to finish one episode
        nstep_returns: list[float] = []
        # to track the n_step (fast) intrinsic reward
        # (episodic returns are the same, we simply have enough steps to complete an episode)
        ...
        # we use the episodic stats if we completed at least one episode, else we use the n_step ones
        returns = episode_returns if num_collected_episodes > 0 else nstep_returns
        lens = episode_lens if num_collected_episodes > 0 else [step_count]
        return IntrinsicCollectStats.with_autogenerated_stats(
            returns=np.array(returns),
            int_returns=np.array(nstep_intrinsic_returns),
            lens=np.array(lens),
