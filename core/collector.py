...
class GoalCollectStats(CollectStats):
    ...
    def with_autogenerated_stats(
        ...
        instance = CollectStats.with_autogenerated_stats(
            returns,
            lens,
        ...
        instance.int_returns = int_returns
        return instance
...
class GoalCollector(Collector):
    ...
    def _collect(
        ...
        # to track the (extrinsic) reward if we do not make enough steps to finish one episode
        nstep_returns: list[float] = []
        # to track the n_step intrinsic reward
        ...
        while True:
            ...
            # (note that rew_R is the extrinsic reward!)
            ...
            int_rew_R = self.policy.self_model.compute_intrinsic_reward(
                current_iteration_batch
            )
            current_iteration_batch.int_rew = int_rew_R
        ...
        # we use the episodic returns if available, else we use the n_step ones
        returns = episode_returns if episode_returns else nstep_returns
        intrinsic_returns = (
            episode_intrinsic_returns
            if episode_intrinsic_returns
            else nstep_intrinsic_returns
        )
        return GoalCollectStats.with_autogenerated_stats(
            returns=np.array(returns),
            int_returns=np.array(intrinsic_returns),
