...
class IntrinsicHERReplayBuffer(HERReplayBuffer):
    ...
    def __init__(
        self,
        size: int,
        # TODO this should probably be in intrinsic/, do I want to keep this as an argument?
        compute_reward_fn: Callable[[np.ndarray, np.ndarray], np.ndarray],
        horizon: int,
        future_k: float = 8,
        **kwargs: Any
    ) -> None:
        super().__init__(size, compute_reward_fn, horizon, future_k, **kwargs)
